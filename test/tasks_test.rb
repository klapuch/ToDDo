...
class Tasks < Minitest::Test
	def test_incrementing_order
		tasks = Toddo::Tasks.new()
			.put('Indent')
			.put('Refactor')
			.to_a
	...
	def test_removing_by_known_order
		tasks = Toddo::Tasks.new(['foo', 'bar', 'baz', 'bur'])
			.remove(2)
			.to_a
	...
	def test_removing_in_natural_order
		assert_nothing_raised do
			Toddo::Tasks.new(['foo', 'bar', 'baz', 'bur']).remove(4)
		end
		assert_nothing_raised do
			Toddo::Tasks.new(['foo', 'bar', 'baz', 'bur']).remove(1)
		end
	end
	...
	def test_throwing_on_removing_by_unnatural_order
		...
		zero_exception = assert_raises RangeError do
			Toddo::Tasks.new(['foo', 'bar', 'baz', 'bur']).remove(0)
		end
	...
	def test_throwing_on_removing_over_limit
		overflow_exception = assert_raises RangeError do
			Toddo::Tasks.new(['foo', 'bar', 'baz', 'bur']).remove(5)
		end
